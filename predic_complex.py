...
7星彩号码智能预测系统 v6.0
═══════════════════════════
13种算法集成预测 — 纯PyTorch多模型版

基于 v5.1 修改:
  ✅ 完全移除 TensorFlow 依赖
  ✅ PyTorch Transformer 替代 TF-Attention
  ✅ 新增 TCN (时间卷积网络)
  ✅ 新增 MoE (混合专家模型)
...
保留修复清单:
  ...
  🟡 #9  默认值矛盾 → 修正逻辑
  🟡 #10 重复导入 → 统一顶部
  🟢 #11 Transformer-Attention → PyTorch实现
...
def method_delta_trend(history, n_predict=5, window=8):
    ...
    for _ in range(n_predict):
        ...
        for pos in range(POS_COUNT):
            nv = POS_RANGES[pos]
            deltas = [recent[j]['numbers'][pos] - recent[j - 1]['numbers'][pos]
                      for j in range(1, len(recent))]
            if not deltas:
                pred.append(last[pos])
                continue
...
if HAS_TORCH:
    ...
    class Generator(nn.Module):
        ...
        def encode_condition(self, seq):
            xl = seq.long()
            embs = [self.cond_emb[p](xl[:, :, p].clamp(0, POS_RANGES[p] - 1))
                    for p in range(POS_COUNT)]
...
if HAS_TORCH:
    ...
    class PolicyNetwork(nn.Module):
        def __init__(self, state_dim, hidden=128):
            ...
            self.heads = nn.ModuleList([nn.Linear(hidden, POS_RANGES[p])
                                        for p in range(POS_COUNT)])
    ...
    def train_rl_policy(train_data, val_data=None, lookback=10, epochs=300, verbose=True):
        ...
        for i in range(lookback + 1, len(train_data)):
            ...
            if feat is not None:
                states.append(feat)
                targets.append(train_data[i]['numbers'])
        if len(states) < 10:
            if verbose:
                print("      ⚠ RL 样本不足")
            return None, None, None
        ...
        if val_data and len(val_data) > lookback + 1:
            ...
            for i in range(lookback + 1, len(val_data)):
                ...
                if feat is not None:
                    vs.append(feat)
                    vt.append(val_data[i]['numbers'])
            if vs:
                val_states = np.array(vs)
                val_targets = np.array(vt)
        ...
        for epoch in range(epochs):
            ...
            with torch.no_grad():
                ...
                rwd = sum((torch.argmax(probs[p], -1) == tgt[:, p]).float()
                          for p in range(POS_COUNT))
                adv = rwd - rwd.mean()
            ...
            if val_states is not None:
                ...
                with torch.no_grad():
                    ...
                    vlp = policy(vs_t)
                    val_loss = sum(F.nll_loss(vlp[p], vt_t[:, p]).item()
                                   for p in range(POS_COUNT))
                monitor = val_loss
            else:
                monitor = total.item()
...
# ############################################################
#  🆕 模型 11-NEW：PyTorch Transformer Attention
#     (替代原 TF-Attention，功能完全对等)
# ############################################################
...
if HAS_TORCH:
    ...
    class TransformerAttentionModel(nn.Module):
        """
        PyTorch Multi-Head Transformer — 替代 TF-Attention
        ────────────────────────────────────────────────────
        结构: Embedding → 投影 → PosEnc → 2层TransformerEncoder
              → 注意力池化 → 7个分类头
        """

        def __init__(self, seq_len=5, d_model=64, n_heads=4,
                     ff_dim=128, n_layers=2, dropout=0.3):
            super().__init__()
            self.emb_dim = 8
            ...
            # 投影到 d_model
            self.project = nn.Linear(POS_COUNT * self.emb_dim, d_model)
            ...
            # 注意力池化 (学习哪个时间步更重要)
            self.attn_pool_w = nn.Linear(d_model, 1)
        ...
        def forward(self, x):
            ...
            emb = torch.cat([
                ...
                for p in range(POS_COUNT)
            ], dim=-1)  # (B, seq, POS_COUNT*emb_dim)

            h = self.project(emb)  # (B, seq, d_model)
            h = self.pos_enc(h)
            ...
            # 注意力池化
            aw = torch.softmax(self.attn_pool_w(h), dim=1)  # (B, seq, 1)
            h = (h * aw).sum(dim=1)  # (B, d_model)
...
if HAS_TORCH:
    ...
    class CausalConv1dBlock(nn.Module):
        ...
        def __init__(self, in_ch, out_ch, kernel_size=3, dilation=1, dropout=0.3):
            super().__init__()
            padding = (kernel_size - 1) * dilation  # 左侧填充以保证因果
    ...
    class TCNModel(nn.Module):
        ...
        ──────────────
        优势: 并行化、长程依赖、无梯度消失
        结构: Embedding → 膨胀因果卷积堆叠 → 全局池化 → 分类头
        """

        def __init__(self, hidden_size=64, n_blocks=3, kernel_size=3, dropout=0.3):
            super().__init__()
            self.emb_dim = 8
            in_dim = POS_COUNT * self.emb_dim

            self.embeddings = nn.ModuleList([
                nn.Embedding(POS_RANGES[p], self.emb_dim) for p in range(POS_COUNT)])

            # 膨胀因果卷积块：dilation = 1, 2, 4
            blocks = []
            ch = in_dim
            for i in range(n_blocks):
                dilation = 2 ** i
                out_ch = hidden_size
                blocks.append(CausalConv1dBlock(ch, out_ch, kernel_size, dilation, dropout))
                ch = out_ch
        ...
        def forward(self, x):
            ...
            h = emb.permute(0, 2, 1)  # (B, in_dim, seq) for Conv1d
            h = self.tcn(h)           # (B, hidden, seq)
            h = h[:, :, -1]           # 取最后一个时间步 (B, hidden)
            h = self.dropout(h)
...
if HAS_TORCH:
    ...
    class MoEModel(nn.Module):
        ...
        def __init__(self, hidden_size=64, n_experts=4, top_k=2,
            ...
            # 序列编码: 双向GRU
            self.encoder = nn.GRU(in_dim, hidden_size, num_layers,
                                  batch_first=True,
                                  dropout=dropout if num_layers > 1 else 0,
                                  bidirectional=True)
            ...
            self.experts = nn.ModuleList([
                Expert(enc_dim, hidden_size, hidden_size, dropout)
        ...
        def forward(self, x):
            ...
            h = enc_out[:, -1, :]  # (B, enc_dim)

            # 门控: 选 Top-K 专家
            gate_logits = self.gate(h)  # (B, n_experts)
            gate_topk_val, gate_topk_idx = torch.topk(gate_logits, self.top_k, dim=-1)
            gate_weights = F.softmax(gate_topk_val, dim=-1)  # (B, top_k)

            # 专家前向
            expert_outputs = torch.stack([expert(h) for expert in self.experts],
                                         dim=1)  # (B, n_experts, hidden)

            # 加权融合 Top-K 专家
            batch_size = h.size(0)
            selected = torch.gather(
                expert_outputs, 1,
                gate_topk_idx.unsqueeze(-1).expand(-1, -1, expert_outputs.size(-1))
            )  # (B, top_k, hidden)
            mixed = (selected * gate_weights.unsqueeze(-1)).sum(dim=1)  # (B, hidden)

            mixed = self.norm(mixed)
            mixed = self.dropout(mixed)

            return [head(mixed) for head in self.heads]
...
class RandomForestCache:
    ...
    def train(self, train_data, lookback=10):
        ...
        for i in range(lookback + 1, len(train_data)):
            ...
            if feat is not None:
                X_l.append(feat)
                y_l.append(train_data[i]['numbers'])
        if len(X_l) < 20:
            return False
...
# ############################################################
#  通用 RNN/Transformer/TCN/MoE 训练器
# ############################################################
...
if HAS_TORCH:
    ...
    def train_rnn_model(model_class, train_data, val_data, seq_len=5,
                        ...
                        **model_kwargs):
        X_tr, y_tr = make_sequences(train_data, seq_len)
        X_val, y_val = make_sequences(val_data, seq_len) if val_data else (
            np.array([]), np.array([]))
        if len(X_tr) < 3:
            if verbose:
                print(f"    ⚠ {model_name} 样本不足")
            return None, None

        device = get_device()

        # 根据模型类型传递正确的参数
        if model_class in (TransformerAttentionModel,):
            model = model_class(seq_len=seq_len, **model_kwargs).to(device)
        elif model_class in (TCNModel, MoEModel):
        ...
        if verbose:
            print(f"      [{model_name}{model_id}] 参数量: {n_params:,}")
    ...
    def method_rnn_ensemble(model_class, model_name, train_data, val_data,
        ...
        for mi in range(n_models):
            ...
            torch.manual_seed(42 + mi * 7)
            np.random.seed(42 + mi * 7)
            model, device = train_rnn_model(
                model_class, train_data, val_data, seq_len, 300, 0.003,
                True, model_name, mi + 1, d_hash, **model_kwargs)
            if model is None:
                continue
...
def ensemble_predict(history, train_data, val_data, n_predict=5,
                     n_rounds=200, d_hash=""):
    stat_methods = {
    ...
    # ⑧ Transformer Attention (PyTorch — 替代 TF-Attention)
    transformer_preds_out = None
    if HAS_TORCH:
        print(f"\n  ── ⑧ Transformer Attention (3模型) ──")
        t0 = time.time()
        try:
            sq = min(5, len(train_data) - 2)
            tp, tpr = method_rnn_ensemble(
                TransformerAttentionModel, "Transformer", train_data, val_data,
                history, n_predict, sq, 3, d_hash,
                d_model=64, n_heads=4, ff_dim=128, n_layers=2, dropout=0.3)
            print(f"\n    ✓ Transformer ({time.time() - t0:.1f}s)")
            all_method_preds['Transformer'] = tp[0]
            transformer_preds_out = tp
            for step in range(n_predict):
                for pos in range(POS_COUNT):
    ...
    if HAS_TORCH:
        ...
        try:
            sq = min(5, len(train_data) - 2)
            tcp, tcpr = method_rnn_ensemble(
                TCNModel, "TCN", train_data, val_data,
                history, n_predict, sq, 3, d_hash,
                hidden_size=64, n_blocks=3, kernel_size=3, dropout=0.3)
            print(f"\n    ✓ TCN ({time.time() - t0:.1f}s)")
            all_method_preds['TCN'] = tcp[0]
            tcn_preds_out = tcp
            for step in range(n_predict):
                for pos in range(POS_COUNT):
                    vote_matrix[step][pos] += tcpr[step][pos] * 1.8 * n_rounds
        except Exception as e:
            print(f"    ⚠ TCN: {e}")
    ...
    if HAS_TORCH:
        ...
        try:
            sq = min(5, len(train_data) - 2)
            mp, mpr = method_rnn_ensemble(
                MoEModel, "MoE", train_data, val_data,
                history, n_predict, sq, 3, d_hash,
                hidden_size=64, n_experts=4, top_k=2, num_layers=2, dropout=0.3)
            print(f"\n    ✓ MoE ({time.time() - t0:.1f}s)")
            all_method_preds['MoE'] = mp[0]
            moe_preds_out = mp
            for step in range(n_predict):
                for pos in range(POS_COUNT):
                    vote_matrix[step][pos] += mpr[step][pos] * 1.8 * n_rounds
        except Exception as e:
            print(f"    ⚠ MoE: {e}")

    if not HAS_TORCH:
        print(f"\n  ⓘ PyTorch 未安装，跳过所有深度学习模型")
    ...
    return (final_predictions, confidence_matrix, vote_matrix,
            ...
            transformer_preds_out, tcn_preds_out, moe_preds_out,
            all_method_preds)
...
def save_log(history, predictions, confidences, all_method_preds,
    ...
    with open(log_path, 'w', encoding='utf-8') as f:
        f.write(f"{'=' * 80}\n")
        f.write(f"7星彩预测系统 v6.0 — 完整日志 (纯PyTorch多模型版)\n")
        ...
        for name, pred in all_method_preds.items():
            if pred is not None:
                ...
                f.write(f"  {name:>12}: [{ps}] 和值={sum(pred)}\n")
        ...
        for pos in range(POS_COUNT):
            ...
            f.write(f"  位置{pos + 1}: " +
                    " ".join(f"{i}({p[i] * 100:.1f}%)" for i in top5) + "\n")
...
def main():
    # 算法计数: 6统计 + 贝叶斯 + RF + LSTM + GRU + GAN + RL + Transformer + TCN + MoE
    n_algo = 6 + 1 + (1 if HAS_SKLEARN else 0) + (7 if HAS_TORCH else 0)

    print()
    print("╔══════════════════════════════════════════════════════════════════════╗")
    print("║       7 星 彩 号 码 智 能 预 测 系 统  v6.0                        ║")
    print("║    多模型集成预测 — 纯 PyTorch 版 (无需TensorFlow)                 ║")
    print("╠══════════════════════════════════════════════════════════════════════╣")
    ...
    print("╠══════════════════════════════════════════════════════════════════════╣")
    if HAS_TORCH:
        print("║  深度模型: LSTM | GRU | GAN | RL | Transformer | TCN | MoE        ║")
    print("║  统计模型: 频率 | 热冷 | 马尔可夫 | 加权 | 趋势 | 模式 | 贝叶斯   ║")
    if HAS_SKLEARN:
        print("║  机器学习: 随机森林                                                ║")
    print("╚══════════════════════════════════════════════════════════════════════╝")
    ...
    (predictions, confidences, vote_matrix,
     ...
     transformer_preds, tcn_preds, moe_preds,
     all_method_preds) = result
    ...
    print(f"  🔬 各算法独立结果（第1期）")
    print(f"{'═' * 70}")
    method_order = ['频率分析', '热号冷号', '马尔可夫链', '加权近期', '差值趋势',
                    '模式匹配', '贝叶斯', '随机森林',
                    'LSTM', 'GRU', 'GAN', 'RL', 'Transformer', 'TCN', 'MoE']
    ...
    for pos in range(POS_COUNT):
        ...
        print(f"  位置{pos + 1}(0~{nv - 1:>2}): "
              f"首选={t1}({t1p:.0f}%) 一致={cons:.2f} {bar} {lb}")

    # 模型覆盖度统计
    print(f"\n{'═' * 70}")
    print(f"  📈 模型覆盖度统计")
    print(f"{'═' * 70}")
    n_active = len(all_method_preds)
    print(f"  活跃模型: {n_active}/{n_algo}")
    deep_models = [n for n in ['LSTM', 'GRU', 'GAN', 'RL', 'Transformer', 'TCN', 'MoE']
                   if n in all_method_preds]
    stat_models = [n for n in ['频率分析', '热号冷号', '马尔可夫链', '加权近期',
                                '差值趋势', '模式匹配', '贝叶斯']
                   if n in all_method_preds]
    ml_models = [n for n in ['随机森林'] if n in all_method_preds]
    print(f"  统计方法: {len(stat_models)} — {', '.join(stat_models)}")
    print(f"  深度学习: {len(deep_models)} — {', '.join(deep_models)}")
    print(f"  机器学习: {len(ml_models)} — {', '.join(ml_models)}")
